...
function ini(){
	...
	for(var i=0;i<pagesDiv.childNodes.length;i++){
		if(pagesDiv.childNodes[i].nodeType == 1){ //element nodes only
			...
			document.getElementById("myBook").appendChild(newElement);

			if(pageShadow){createShadow(newElement,j);}; //shadow function
			if(j>0){createList(newElement,j);}; //create dropdown navigation list


			pages[j] = document.getElementById("myBook").innerHTML;
			//alert(pages[j]);
			document.getElementById("myBook").removeChild(newElement);
			j++;
		}
	}
	if(myBook=document.getElementById("myBook")){

		const width = (pWidth*2)+(pBorders*2)
		const height = (pHeight+pBorders*1.5)
		myBook.style.width=(width)+'px';
		myBook.style.height=(height)+'px';
		document.getElementById('television').style.width=width+'px';
		document.getElementById('television').style.height=height+'px';
		...
		for(i=4;i>=1;i--){
			...
			myDiv.style.top=pBorders/3+"px";
			myDiv.style.width=(pWidth)+"px";
			myDiv.style.height=(pHeight)+"px";
			myDiv.style.padding= pageBorderWidth + "px";
			...
			if (i == 1 || i == 3 ){
				myDiv.setAttribute("class","page left");
				myDiv.style.left=(pBorders/2)+'px';
			} else {
				myDiv.setAttribute("class","page right");
				myDiv.style.left=(pWidth+pBorders/1.5)+'px';
			}
...
function goleft(currLeft,currWidth){
	if(currLeft >=(pWidth-(numPixels*2))){
		...
		p2.style.left=(pWidth+pBorders/1.5)+"px";
